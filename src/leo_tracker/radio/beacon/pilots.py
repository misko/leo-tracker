    ...
    while epoch_sample + round(frame * period) < values.size:
        ...
        for symbol in range(2, 302):
            ...
            if start < 0 or start + count > values.size or count < 2:
                continue
            local = template[local_start:local_start + count]
            sample_indexes = np.arange(start, start + count)
            corrected = values[start:start + count] * np.exp(
                -2j * np.pi * frequency_offset_hz * sample_indexes / sample_rate_hz)
            correlation = np.vdot(local, corrected)
            denominator = float(np.vdot(local, local).real * np.vdot(corrected, corrected).real)
            correlations.append(correlation)
            powers.append(0.0 if denominator == 0 else abs(correlation) ** 2 / denominator)
